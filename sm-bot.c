...
// T_eval minimax(T_Super_Morpion position, int depth, int isMax, enum T_Couleur firstPlayer)
// {
//     if (depth == 0)
//     {
...
//         T_eval childEval = minimax(newPosition, depth - 1, 1 - isMax, firstPlayer);
...
void fillPositionFromFen(T_Super_Morpion *position, char *fen) {
    ...
    for (int i = 0; i < 9; i++) {
        if (position->grilles[i] != VIDE) {
            for (int j = 0; j < 9; j++) {
                position->cases[i * 9 + j] = position->grilles[i];
            }
        }
    }
    fenIndex++;
    ...
    fenIndex++;
    position->trait = fen[fenIndex] == 'x' ? NOIR : BLANC;
}
...
void printSuperMorpion(T_Super_Morpion *position)
{
    ...
    printf("Evaluation de la position : %d\n", position->trait == NOIR ? evaluate(position) : -evaluate(position));
    printf("Dernier coup joué : %s\n", convertIndexToMove(position->lastCoupId));
}
...
int main(int argc, char **argv)
{
    ...
    // T_eval bestMove = minimax(game, DEFAULT_MINIMAX_DEPTH, 1, game.trait);
    T_eval bestMove = negamax(game, DEFAULT_MINIMAX_DEPTH, -1000, 1000, game.trait);
    printf("%s", convertIndexToMove(bestMove.moveId));
